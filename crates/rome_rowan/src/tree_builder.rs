...
impl<L: AstTreeShape> TreeBuilder<'_, L> {
	...
	pub fn finish_node(&mut self) {
		...
		// TODO only check if not already cached
		let children = &self.children[first_child..];
		let child_kinds = children
			.iter()
			.map(|(_, element)| element.as_ref().map(|e| L::kind_from_raw(e.kind())));

		let kind = if L::fits_shape_of(&kind, children.len(), child_kinds) {
			kind
		} else {
			kind.to_unknown()
		};
	...
	/// Example:
	/// ```rust
	/// # use rome_rowan::{TreeBuilder, RawKind};
