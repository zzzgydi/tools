...
use rome_analyze::{AnalysisAPI, FileId, Signal, TextAction};
...
async fn analyze_file(
	...
	let mut analysis_api = AnalysisAPI::default();
	analysis_api.set_file_text(file_id, text);

	let mut actions: Vec<TextAction> = vec![];
	let mut diagnostics: Vec<Diagnostic> = vec![];

	for analysis in analysis_api.analyze(file_id) {
...
pub async fn run_server(stdin: Stdin, stdout: Stdout) {
	let (service, messages) = LspService::new(|client| LSPServer::new(client));
